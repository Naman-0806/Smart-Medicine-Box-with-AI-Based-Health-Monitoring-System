...
from firebase.auth_service import logout_user
...
def render_sidebar():
    ...
    # Synchronize authentication session variables
    ...
    patient = get_patient_by_id(user_uid, owner_uid=user_uid)
    if patient:
        st.sidebar.markdown(f"👤 Profile: **{patient.get('name') or patient.get('full_name')}**")
        st.sidebar.caption(f"ID: `{user_uid}`")
    else:
